...
_loc1.addListeners = function()
{
   ...
   // Listen to ogrines changes for instant UI update
...
_loc1.playerOgrineChange = function(oEvent)
{
   // Update ogrinas label instantly when datacenter value changes
   var ogrineValue = oEvent.value;
...
_loc1.yes = function(_loc2_)
{
   ...
   {
      this.api.network.send("!1C" + _loc2_.target.params.id);
   }
   else
   {
      this.api.network.send("!1B" + _loc2_.target.params.id);
   }
};
...
_loc1.click = function(oEvent)
{
   switch(oEvent.target)
   {
      ...
      case this.VentaOgrinas:
         ...
         {
            this.api.network.send("!1A" + this.Misogrinas.text + "|" + this.precioKamas.text);
         }
      default:
         return;
   }
