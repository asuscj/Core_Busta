...
_loc1.updateItemCount = function()
{
   ...
   var _loc4_ = 0;
   var _loc3_;
   for(_loc5_ in this._oData.inventory)
   {
      _loc3_ = this._oData.inventory[_loc5_];
      _global.API.kernel.debugConsole(_loc4_ + " " + _loc3_.price);
      _loc4_ += Number(_loc3_.price);
      _loc5_ = _loc5_ + 1;
   }
   this._lblTotalVenta.text = this.api.lang.getText("TOTAL_VENT") + ": " + new ank["\x1e\n\x07"]["\x0e\x1c"](_loc4_).addMiddleChar(this.api.lang.getConfigText("THOUSAND_SEPARATOR"),3) + " " + this.api.lang.getText("KAMAS");
};
_loc1.addListeners = function()
{
   ...
   this._dgPricesNew.addEventListener("itemSelected",this);
   ...
   this.api.kernel.debugAstra("addListeners");
   this.api.kernel.KeyManager.addShortcutsListener("onShortcut",this);
};
_loc1.setQuantityHeader = function()
{
   this._dgPricesNew.columnsNames = ["","x" + this._oData.quantity1,"x" + this._oData.quantity2,"x" + this._oData.quantity3];
};
_loc1.hideItemViewer = function(_loc2_)
{
   ...
   this._dgPricesNew._visible = !_loc2_;
...
{
   this._dgPricesNew.dataProvider = oItem;
};
_loc1.selectPrice = function(_loc2_)
{
...
_loc1.click = function(_loc2_)
{
   ...
   switch(_loc2_.target._name)
   {
      case "_btnRemove":
         ...
         break;
      case "_btnAdd":
         _loc5_ = Number(this._tiPrice.text);
         _loc4_ = Number(this._cbQuantity.selectedItem.index);
         if(_global.isNaN(_loc5_) || _loc5_ == 0)
         {
         ...
         {
            this.askSell(this._oSelectedItem,_loc4_,_loc5_);
         }
         break;
      case "_btnClose":
         this.callClose();
